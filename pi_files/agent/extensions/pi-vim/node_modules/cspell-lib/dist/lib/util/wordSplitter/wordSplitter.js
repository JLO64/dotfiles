...
export function split(line, offset, isValidWord, options = {}) {
    ...
    try {
        const _x = __addDisposableResource(env_1, measurePerf('wordSplitter.split'), false);
        ...
        function rebaseTextOffsetToAbsolute(relText) {
            relText.offset += lineOffset;
            return relText;
        }
        function rebaseTextOffsetToRelative(relText) {
            relText.offset -= lineOffset;
            return relText;
        }
        function createTextOffsetWithValid(start, end, isFound) {
            const word = {
                offset: start,
                length: end - start,
        ...
        function calcKey(start, end) {
            const i = start;
            const j = end - start;
            const v = i + (j << 20);
            return i < 1 << 20 && j < 1 << 11 ? v : -1;
        }
        function checkTextRange(start, end) {
...
function removeSoftHyphen(word) {
...
function splitIntoWords(lineSeg, breaks, checkTextRange) {
    ...
    const terminalNode = { n: undefined, i: maxIndex, j: maxIndex, c: 0, nc: 0, text: undefined };
    ...
    function findNearestBreakIndex(offset, bi) {
     ...
     */
    function makeCandidates(p, i, bi, bs, currentCost) {
    ...
    function addToKnownPaths(candidate) {
        ...
        for (let can = candidate; can !== undefined; can = can.p) {
            ...
            const node = { n: path, i, j, c: cost, nc, text: t };
