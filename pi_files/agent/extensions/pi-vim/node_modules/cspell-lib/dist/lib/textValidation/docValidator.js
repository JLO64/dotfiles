...
import { chainTransformers, createIntlSegmentTextTransformer, createMappedTextSegmenter, createSoftWordBreakTextTransformer, createSubstitutionTransformer, } from '../Transform/index.js';
...
export class DocumentValidator {
    ...
    async _prepareAsync() {
        ...
        const { transformer, validateOptions, includeRanges, segmenter, textValidator } = this.#calcSharedPrep(finalSettings, dict);
        recFinalizeTime();
        this._preparations = {
            config,
            dictionary: dict,
    ...
    #calcSharedPrep(finalSettings, dict) {
        const sub = createSubstitutionTransformer(finalSettings);
        if (sub.missing) {
            this.addPossibleError(`Missing substitutions: ${sub.missing.join(', ')}`);
        }
        let transformer = sub.transformer;
        if (finalSettings.useIntlWordSegmentation) {
            transformer = chainTransformers(transformer, createIntlSegmentTextTransformer(finalSettings.language || ''));
        }
